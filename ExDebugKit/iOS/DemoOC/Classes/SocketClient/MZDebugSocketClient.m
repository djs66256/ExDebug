...
@interface MZDebugSocketPackageData : NSObject
@property (nonatomic, strong) NSData *data;
@property (nonatomic, assign) CFIndex offset;
@end
@implementation MZDebugSocketPackageData

@end

@interface MZDebugSocketClient () {
...
static void DebugSocketClientReadStreamClientCallBack(CFReadStreamRef stream, CFStreamEventType type, void *clientCallBackInfo) {
    if ([(__bridge id)clientCallBackInfo isKindOfClass:[MZDebugSocketClient class]]) {
        MZDebugSocketClient *client = (__bridge MZDebugSocketClient *)clientCallBackInfo;
        switch (type) {
...
@implementation MZDebugSocketClient

/*
+ (instancetype)sharedInstance {
    static MZDebugSocketClient *client = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        client = [MZDebugSocketClient new];
    });
    return client;
}
 */
...
- (void)_receiveData:(NSData *)data {
    ...
    do {
        int code = 0;
        MZDebugSocketMessage *msg = [MZDebugSocketMessage readFromData:data offset:&offset errorCode:&code];
        if (msg) {
            readBuffer = nil;
...
- (void)_sendMessage:(MZDebugSocketMessage *)message {
    if (self.connected) {
...
- (void)_writeData:(NSData *)data {
    if (writePackageDatas.count == 0 && CFWriteStreamCanAcceptBytes(writeStream)) {
        ...
        if (index == -1) {
            ...
            MZDebugSocketPackageData *package = [MZDebugSocketPackageData new];
            package.data = data;
...
- (void)_writeNextData {
    if (writePackageDatas.count > 0) {
        MZDebugSocketPackageData *package = writePackageDatas.firstObject;
