...
@implementation DDDebugSocketMessage
@synthesize head = _head;

+ (DDDebugSocketMessage *)requestWithPath:(NSString *)path {
    return [[DDDebugSocketMessage alloc] initWithType:DDDebugSocketMessageTypeRequest path:path head:nil body:nil];
}

+ (DDDebugSocketMessage *)registerWithPath:(NSString *)path {
    return [[DDDebugSocketMessage alloc] initWithType:DDDebugSocketMessageTypeRegister path:path head:nil body:nil];
}

+ (DDDebugSocketMessage *)readFromData:(NSData *)data offset:(inout NSInteger *)offsetPtr errorCode:(out int *)code {
    NSInteger offset = *offsetPtr;
    NSString *sipc = [data readStringWithLength:4 offset:offset];
    offset += 4;
    if ([sipc isEqualToString:@"SIPC"]) {
        DDDebugSocketMessage *msg = [[DDDebugSocketMessage alloc] init];
        // type
...
- (instancetype)initWithType:(DDDebugSocketMessageType)type path:(NSString *)path head:(NSDictionary *)head body:(NSDictionary *)body
{
    self = [super init];
    if (self) {
        self.type = type;
...
- (DDDebugSocketMessage *)makeReplyMessage {
    DDDebugSocketMessage *reply = [[DDDebugSocketMessage alloc] initWithType:self.type path:self.path head:self.head body:nil];
    reply.id = self.id;
    return reply;
}

- (DDDebugSocketMessage *)buildReply:(void (^)(DDDebugSocketMessage *))block {
    DDDebugSocketMessage *reply = [[DDDebugSocketMessage alloc] initWithType:self.type path:self.path head:self.head body:nil];
    reply.id = self.id;
    block(reply);
    return reply;
...
- (NSString *)debugDescription {
    ...
    return [NSString stringWithFormat:@"<SIPC Message> %@ %d (%@)\n head: %@\n body: %@",
            self.type == DDDebugSocketMessageTypeRequest ? @"REQUEST" : @"RESPONSE",
            self.id, self.path,
...
@implementation NSString (DDDebugSocketMessageDataValue)
...
@implementation NSDictionary (DDDebugSocketMessageDataValue)
- (NSString *)contentType {
    return @"json";
}
- (NSData *)NSDataValue {
...
@implementation NSArray (DDDebugSocketMessageDataValue)
- (NSString *)contentType {
    return @"json";
}
- (NSData *)NSDataValue {
...
@implementation NSMutableData (DDDebugSocketMessage)
...
@implementation NSData (DDDebugSocketMessage)
